  ...
  return (
    <div className="relative">
      ...
      <div className="absolute inset-0 z-50">
        ...
        <LoaderCircleIcon className="absolute left-[calc(50%-0.75rem)] top-[calc(50%-0.75rem)] size-6 animate-spin text-blue-700" />
      </div>
    </div>
  )
}
...
const StatusBorder = ({ children, className }: { children: ReactNode; className?: string }) => {
  return (
    <>
      <div
        className={cn(
          'absolute -left-0.5 -top-0.5 h-[calc(100%+4px)] w-[calc(100%+4px)] rounded-[8px] border-2',
          className,
        )}
      />
      {children}
...
export const NodeStatusIndicator = ({
  ...
  switch (status) {
    ...
    case 'success':
      return (
        <StatusBorder className={cn('dark:border-emerald-600 border-green-700/50', className)}>
          {children}
        </StatusBorder>
      )
    case 'error':
      return (
        <StatusBorder className={cn('dark:border-red-400 border-red-700/50', className)}>
          {children}
        </StatusBorder>
      )
    default:
